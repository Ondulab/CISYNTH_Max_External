...
{
    socklen_t addrlen = sizeof(t_sysaddr);
#if defined(MAC_VERSION) && !TARGET_RT_MAC_MACHO
...
ssize_t syssock_recv(t_syssocket sockfd, void *buf, ssize_t len, unsigned int flags)
{
#if defined(MAC_VERSION) && !TARGET_RT_MAC_MACHO
...
ssize_t syssock_send(t_syssocket sockfd, const void *buff, size_t nbytes, int flags)
{
#if defined(MAC_VERSION) && !TARGET_RT_MAC_MACHO
...
ssize_t syssock_recvfrom(t_syssocket sockfd, void *buf, ssize_t len, unsigned int flags, struct sockaddr *from,int *fromlen)
{
#if defined(MAC_VERSION) && !TARGET_RT_MAC_MACHO
...
ssize_t syssock_sendto(t_syssocket sockfd, const void *buff, size_t nbytes, int flags, const struct sockaddr *to, int tolen)
{
#if defined(MAC_VERSION) && !TARGET_RT_MAC_MACHO
...
ssize_t syssock_writechunks(t_syssocket sockfd, data_chunk *chunk, int count)
{
#ifdef MAC_VERSION
	#if TARGET_RT_MAC_MACHO
